...
class CAC_Artritis {
    ...
    public getOne(C8_CAMPO_9) {
        return new Promise(function (resolev, reject) {
            try {
                pool.query("SELECT * FROM cuenta_artritis where C8_CAMPO_9 = ? ", [C8_CAMPO_9], function (err, result, fields) {
                    if (err) throw err;
                    resolev(result)
                });
            }
    ...
    public actualizarDatos(newDatos, C8_CAMPO_9) {
        return new Promise(function (resolev, reject) {
            try {
                pool.query('UPDATE cuenta_artritis set ? where cuenta_artritis.C8_CAMPO_9 = ? ', [newDatos, C8_CAMPO_9], function (err, result, fields) {
                    if (err) throw err;
                    resolev(result)
                });
            }
